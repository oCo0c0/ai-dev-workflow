...
import type {MinerUService, MinerUParseOptions, MinerUBackend, MinerUParseMethod} from '../services/mineru-service.js';
 ...
 */
export function createMinerURoutes(mineruService: MinerUService): Router {
    const router = Router();
    ...
    router.get('/status', async (_req, res) => {
        try {
            ...
            res.status(500).json({code: 'MINERU_ERROR', message: getErrorMessage(err)});
        }
    });
    ...
    router.post('/parse', upload.array('files', 10), async (req, res) => {
        try {
            ...
            res.status(500).json({code: 'MINERU_PARSE_ERROR', message: getErrorMessage(err)});
        }
    });
    ...
    router.post('/tasks', upload.array('files', 10), async (req, res) => {
        try {
            ...
            for (const file of files) {
                const result = await mineruService.submitTaskBuffer(
                    file.originalname,
                    file.buffer,
                    file.mimetype,
                    options,
            ...
            res.status(500).json({code: 'MINERU_TASK_ERROR', message: getErrorMessage(err)});
        }
    });
    ...
    router.get('/tasks/:taskId', async (req, res) => {
        try {
            ...
            res.status(500).json({code: 'MINERU_TASK_STATUS_ERROR', message: getErrorMessage(err)});
        }
    });
    ...
    router.get('/tasks/:taskId/result', async (req, res) => {
        try {
            ...
            res.status(500).json({code: 'MINERU_TASK_RESULT_ERROR', message: getErrorMessage(err)});
        }
    });
...
function extractOptions(body: Record<string, unknown>): MinerUParseOptions {
    ...
    return {
        langList,
        backend: typeof body.backend === 'string' ? body.backend as MinerUBackend : undefined,
        parseMethod: typeof body.parseMethod === 'string' ? body.parseMethod as MinerUParseMethod : undefined,
