 ...
 * 监控系统实现
 */
export class AgentMonitorImpl implements AgentMonitor {
    ...
    record(trace: ExecutionTrace, event: MonitorEvent): void {
        ...
        const step: TraceStep = {
            id: uuidv4(),
            type: event.type as TraceStep['type'],
        ...
        if (event.data.tokensUsed) {
            this.updateTokenUsage(trace, event.data.tokensUsed as number);
        }
    }
    ...
    complete(trace: ExecutionTrace, result: AgentResult): void {
        ...
        console.log(
            `[AgentMonitor] Completed trace ${trace.id}: ` +
    ...
    getStats(agentId?: string): {
        ...
        const qualitySum = completedTraces.reduce((sum, trace) => {
            ...
            const lastStep = trace.steps[trace.steps.length - 1];
            const quality = lastStep?.content?.quality as number | undefined;
     ...
     * 更新 Token 使用情况
     */
    private updateTokenUsage(trace: ExecutionTrace, tokens: number): void {
        // 在最后一步更新 Token 使用情况
     ...
     * 标准化错误对象
     */
    private normalizeError(error: Error): ErrorInfo {
        return {
 ...
 * 创建 Agent 监控实例
 */
export function createAgentMonitor(): AgentMonitorImpl {
