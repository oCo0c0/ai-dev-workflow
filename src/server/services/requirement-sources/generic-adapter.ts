...
import {
    ...
    parseMarkdownRequirementDetail,
    parseMarkdownRequirementList,
} from './parsers.js';
 ...
 */
export class GenericAdapter implements RequirementSourceAdapter {
    readonly id = 'generic';
    ...
    parseDetail(content: unknown): RequirementDetail {
        ...
        if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
            return mapJsonToDetailBase(raw as Record<string, unknown>);
        }
    ...
    parseList(content: unknown): Requirement[] {
        ...
        if (Array.isArray(raw)) {
            return raw.map((item: Record<string, unknown>) => mapJsonToRequirement(item));
        }
        if (typeof raw === 'string') {
