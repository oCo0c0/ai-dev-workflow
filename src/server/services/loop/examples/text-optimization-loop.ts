...
import {LoopConfig, Improvement, OptimizationPlan} from '../types.js';
import {createLoopService} from '../loop-service.js';
...
class TextOptimizer {
     ...
     * 评估文本质量
     */
    async evaluateQuality(text: string): Promise<number> {
     ...
     * 生成改进建议
     */
    generateImprovements(text: string, currentQuality: number): Improvement[] {
     ...
     * 应用改进
     */
    applyImprovements(text: string, improvements: Improvement[]): string {
        let result = text;
 ...
 * 示例 1：自适应循环
 */
export async function adaptiveLoopExample(): Promise<void> {
    ...
    const config: LoopConfig = {
        ...
        maxIterations: 10,
        adaptive: true,
        qualityEvaluator: async (result) => await optimizer.evaluateQuality(result as string),
        costEstimator: (action) => optimizer.estimateCost(action)
    };

    let currentText = 'This is a simple text. It needs improvement.';

    const result = await loopService.executeAdaptiveLoop(
        config,
 ...
 * 示例 2：质量循环
 */
export async function qualityLoopExample(): Promise<void> {
    ...
    const result = await loopService.executeQualityLoop(
        config,
        async (improvements) => {
 ...
 * 示例 3：成本优化
 */
export async function costOptimizationExample(): Promise<void> {
    ...
    // 成本分析
    const analysis = (loopService as any).costOptimizer.analyzeCosts(
        config.tokenBudget!,
        result.tokensUsed,
        result.quality,
 ...
 * 示例 4：自动选择策略
 */
export async function autoStrategyExample(): Promise<void> {
    ...
    const highQualityResult = await loopService.executeOptimalLoop(
        {targetQuality: 0.9, tokenBudget: 10000},
        async (_input: any) => {
    ...
    const lowBudgetResult = await loopService.executeOptimalLoop(
        {targetQuality: 0.7, tokenBudget: 3000},
        async (_input: any) => {
    ...
    const balancedResult = await loopService.executeOptimalLoop(
        {targetQuality: 0.8, tokenBudget: 10000},
        async (_input: any) => {
 ...
 * 运行所有示例
 */
export async function runAllLoopExamples(): Promise<void> {
    console.log('╔════════════════════════════════════════════════════════════╗');
