...
class CostOptionGenerator {
     ...
     * 生成成本选项
     */
    generateOptions(task: any, budget: number): CostOption[] {
        return [
            {
                name: 'fast',
                quality: 0.6,
     ...
     * 计算价值（性价比）
     */
    private calculateValue(quality: number, cost: number, time: number): number {
...
class OptimizationPlanGenerator {
    ...
    generatePlan(
        ...
        _currentQuality: number, // 预留参数，未来可根据当前质量定制优化计划
     ...
     * 映射选项名称到策略
     */
    private mapOptionToStrategy(optionName: string): LoopStrategy {
        switch (optionName) {
            case 'fast':
...
export class CostOptimizer {
    private optionGenerator: CostOptionGenerator;
    private planGenerator: OptimizationPlanGenerator;
    ...
    async execute(
        config: LoopConfig,
        executor: (plan: OptimizationPlan) => Promise<any>
    ): Promise<LoopResult> {
    ...
    private selectBestOption(
        ...
        const maxQualityAffordable = affordableOptions.reduce((max, opt) =>
            opt.quality > max.quality ? opt : max
        );

        // 如果最高质量仍远低于目标，考虑是否值得执行
        if (maxQualityAffordable.quality < targetQuality * 0.7) {
            console.warn(
                `[CostOptimizer] Budget insufficient for target quality ` +
     ...
     * 估算最终质量
     */
    private estimateFinalQuality(result: any, expectedQuality: number): number {
        // 简单实现：基于结果是否成功调整预期质量
    ...
    optimizeBudgetAllocation(
        totalBudget: number,
        stages: Array<{ name: string; minQuality: number; importance: number }>
    ): Array<{ stage: string; budget: number; percentage: number }> {
        // 计算总重要性
...
// 导出 LoopStrategy 类型
type LoopStrategy = 'quick-prototype' | 'quality-optimization' | 'fine-polishing' | 'cost-effective';
