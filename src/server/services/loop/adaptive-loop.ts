...
class StrategySelector {
     ...
     * 选择最优策略
     */
    selectStrategy(iteration: number, quality: number, budget: number, timeLimit?: number): LoopStrategy {
        // 时间紧张 - 使用快速原型策略
     ...
     * 判断是否应该优先速度
     */
    private shouldPrioritizeSpeed(iteration: number, timeLimit: number): boolean {
        // 如果时间已过半但质量还不够，优先速度
...
class LoopExecutor {
    ...
    async executeIteration(
        state: LoopState,
        config: LoopConfig,
        executor: (strategy: LoopStrategy, state: LoopState) => Promise<any>
     ...
     * 更新循环状态
     */
    updateState(state: LoopState, iteration: LoopIteration): void {
...
export class AdaptiveLoop {
    ...
    async execute(
        ...
        while (!state.done && state.iteration < (config.maxIterations || 10)) {
            // 选择策略
            state.strategy = this.selector.selectStrategy(
                state.iteration,
                state.quality,
                (config.tokenBudget || 10000) - state.tokensUsed,
                config.timeLimit ? config.timeLimit - state.timeElapsed : undefined
            );
     ...
     * 检查终止条件
     */
    private checkTerminationConditions(state: LoopState, config: LoopConfig): void {
        // 质量达标
    ...
    getStrategyStats(strategies: LoopStrategy[]): {
        [strategy: string]: { count: number; percentage: number }
    } {
 ...
 * 创建自适应循环实例
 */
export function createAdaptiveLoop(): AdaptiveLoop {
