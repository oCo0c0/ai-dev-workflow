...
import {createAgentsService, AgentWorkflowConfig} from '../agents-service.js';

/**
 * 示例 1：单个 Agent 执行
 */
export async function singleAgentExample(): Promise<void> {
    ...
    if (codeResult.result) {
        const result = codeResult.result as any;
 ...
 * 示例 3：对比不同质量目标
 */
export async function qualityComparisonExample(): Promise<void> {
...
export async function errorHandlingExample(): Promise<void> {
    ...
    const workflowWithFailure: AgentWorkflowConfig = {
