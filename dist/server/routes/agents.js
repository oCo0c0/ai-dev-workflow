...
const agents_service_js_1 = require("../services/agents/agents-service.js");
const agent_store_service_js_1 = require("../services/agent-store-service.js");
const coordinator_service_js_1 = require("../services/agents/coordinator-service.js");
 ...
 */
const agentExecutionCache = new lru_cache_js_1.LruCache(50);
/**
 ...
 * 从缓存或文件存储中查找Agent执行
 */
function findAgentExecution(executionId, agentStore) {
 ...
 */
function persistAgentExecution(execution, agentStore) {
 ...
 * 标记Agent执行为失败状态
 */
function failAgentExecution(execution, error, agentStore) {
...
function createAgentRoutes() {
    const agentStore = new agent_store_service_js_1.AgentStoreService();
    const agentService = new agents_service_js_1.AgentsService();
    const coordinatorService = (0, coordinator_service_js_1.getCoordinatorService)();
    ...
    router.post('/list', async (_req, res) => {
        try {
            ...
            res.status(500).json({ code: 'AGENT_ERROR', message: (0, error_utils_js_1.getErrorMessage)(err) });
        }
    });
    ...
    router.post('/coordinator/execute', async (req, res) => {
        try {
            ...
            if (planOnly) {
                try {
                    const result = await coordinatorService.executeAgentMode(requirement, workspace, { ...context, planOnly: true });
                    ...
                    failAgentExecution(execution, (0, error_utils_js_1.getErrorMessage)(err), agentStore);
                }
                return;
            }
            // 异步执行协调Agent
            ...
            try {
                // 使用协调服务执行
                const result = await coordinatorService.executeAgentMode(requirement, workspace, context || {});
                // 执行成功
                execution.status = 'completed';
                execution.result = {
                    plan: result.plan,
            ...
            res.status(500).json({ code: 'COORDINATOR_ERROR', message: (0, error_utils_js_1.getErrorMessage)(err) });
        }
    });
    ...
    router.get('/coordinator/agents', async (_req, res) => {
        try {
            ...
            res.status(500).json({ code: 'AGENT_LIST_ERROR', message: (0, error_utils_js_1.getErrorMessage)(err) });
        }
    });
    ...
    router.post('/coordinator/recommend', async (req, res) => {
        try {
            ...
            const recommendations = coordinatorService.recommendAgents(taskDescription, minConfidence || 0.3);
            res.json(recommendations);
        }
        catch (err) {
            res.status(500).json({ code: 'RECOMMEND_ERROR', message: (0, error_utils_js_1.getErrorMessage)(err) });
        }
    });
    ...
    router.post('/execute', async (req, res) => {
        try {
            ...
            res.status(500).json({ code: 'EXECUTION_ERROR', message: (0, error_utils_js_1.getErrorMessage)(err) });
        }
    });
    ...
    router.post('/workflow', async (req, res) => {
        try {
            ...
            res.status(500).json({ code: 'WORKFLOW_ERROR', message: (0, error_utils_js_1.getErrorMessage)(err) });
        }
    });
    ...
    router.post('/coordinator/confirm-execution', async (req, res) => {
        try {
            ...
            // 检查是否已有执行计划
            const storedPlan = execution.result;
            if (!storedPlan?.plan) {
            ...
            try {
                const context = execution.inputData.context || {};
                const plan = storedPlan.plan;
                // 存储执行计划用于暂停/恢复
                executionPlans.set(executionId, plan);
                // 创建进度回调
                const onProgress = (step, currentPlan) => {
                    (0, websocket_js_1.broadcast)({
                ...
                // 执行协调Agent，传入进度回调
                const result = await coordinatorService.executeAgentMode(execution.inputData.requirement, execution.inputData.workspace, { ...context, planId: executionId, onProgress });
                // 执行成功
                execution.status = 'completed';
                execution.result = {
                    plan: result.plan,
            ...
            res.status(500).json({ code: 'CONFIRM_EXECUTION_ERROR', message: (0, error_utils_js_1.getErrorMessage)(err) });
        }
    });
    ...
    router.post('/answer', async (req, res) => {
        try {
            ...
            // 找到未回答的问题
            const unansweredQuestion = plan.userQuestions.find((q) => !q.answer);
            ...
            res.status(500).json({ code: 'ANSWER_ERROR', message: (0, error_utils_js_1.getErrorMessage)(err) });
        }
    });
