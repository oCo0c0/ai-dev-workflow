...
exports.createWorkspaceRoutes = createWorkspaceRoutes;
 ...
 */
function openSystemFolderPicker(title) {
    ...
    return new Promise((resolve) => {
        ...
        if (platform === 'win32') {
            ...
            const escaped = safe.replace(/"/g, '');
            const child = (0, child_process_1.spawn)('zenity', ['--file-selection', '--directory', `--title=${escaped}`], {
                stdio: ['ignore', 'pipe', 'pipe'],
            });
            let output = '';
            child.stdout.on('data', (d) => {
 ...
 * ```
 */
function createWorkspaceRoutes(workspaceService) {
    ...
    function getAllowedRoots() {
        ...
        return [os_1.default.homedir(), ...saved];
    }
    /**
     * 校验传入的 workspacePath 是否在允许根目录内。
     * 用于 Git 等会以此为 cwd 执行命令的接口。
     */
    function requireAllowedWorkspace(workspacePath) {
        if (!workspacePath) {
            return { valid: false, error: 'workspacePath is required' };
        }
        return (0, validation_js_1.validateWorkspacePath)(workspacePath, getAllowedRoots());
    }
    // ─── 已保存的工作区 ────────────────────────────────────────────────────────
    /**
    ...
    router.get('/saved', (_req, res) => {
        try {
            ...
            res.status(500).json({ code: 'WORKSPACE_ERROR', message: (0, error_utils_js_1.getErrorMessage)(err) });
        }
    });
    /**
     ...
     */
    router.post('/saved', (0, validation_js_1.validateBody)([{ field: 'path', required: true, type: 'string' }]), (req, res) => {
        try {
            ...
            res.status(400).json({ code: 'WORKSPACE_ERROR', message: (0, error_utils_js_1.getErrorMessage)(err) });
        }
    });
    /**
     ...
     */
    router.post('/select', (0, validation_js_1.validateBody)([{ field: 'path', required: true, type: 'string' }]), async (req, res) => {
        try {
            const workspacePath = req.body.path;
    ...
    router.get('/browse', (req, res) => {
        try {
            ...
            let resolvedPath;
            if (dirPath.trim() === '') {
                resolvedPath = os_1.default.homedir();
            }
            else {
                const check = (0, validation_js_1.validateWorkspacePath)(dirPath, getAllowedRoots());
                if (!check.valid) {
    ...
    router.get('/file', (req, res) => {
        try {
            ...
            const wsCheck = (0, validation_js_1.validateWorkspacePath)(workspacePath, getAllowedRoots());
            if (!wsCheck.valid) {
                res.status(400).json({ code: 'VALIDATION_ERROR', message: wsCheck.error });
                return;
            }
    ...
    router.post('/pick', async (req, res) => {
        try {
            const title = req.body?.title || 'Select Workspace Folder';
