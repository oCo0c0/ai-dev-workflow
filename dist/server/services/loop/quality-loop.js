...
exports.QualityLoop = void 0;
exports.createQualityLoop = createQualityLoop;
/**
 * 质量评估器
 */
class QualityEvaluator {
    ...
    defaultEvaluate(result) {
        ...
        if (result && typeof result === 'object') {
            ...
            if (keys.length > 0)
                score += 0.2;
            if (result.data || result.content || result.output)
                score += 0.1;
        }
     ...
     * 生成改进建议
     */
    generateImprovements(issues, score) {
        const improvements = [];
...
class QualityLoop {
     ...
     * 获取质量报告
     */
    getQualityReport(assessment) {
        const lines = [
            `Quality Score: ${(assessment.score * 100).toFixed(1)}%`,
            '',
            `Issues Found: ${assessment.issues.length}`,
            ...assessment.issues.map(issue => `  - [${issue.severity.toUpperCase()}] ${issue.description}`),
            '',
            `Improvements Suggested: ${assessment.improvements.length}`,
            ...assessment.improvements.map(imp => `  - [${imp.priority.toUpperCase()}] ${imp.description} ` +
     ...
     * 比较两次评估
     */
    compareAssessments(before, after) {
...
exports.QualityLoop = QualityLoop;
/**
 * 创建质量优化循环实例
 */
function createQualityLoop() {
