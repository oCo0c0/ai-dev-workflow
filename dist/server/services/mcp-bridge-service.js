...
class MCPBridgeService {
     ...
     */
    constructor(mcpConfigService, serverName) {
    ...
    async ensureConnected() {
        ...
        try {
            ...
            if (!config) {
                throw new Error(`MCP Server "${this.serverName}" is not configured. Please add it in MCP Management.`);
            }
            ...
            const client = new index_js_1.Client({ name: 'ai-dev-workbench', version: '0.1.0' }, { capabilities: {} });
     ...
     */
    async fetchRequirementDetail(id) {
        try {
            ...
            throw new Error(`Failed to fetch requirement detail for "${id}": ${(0, error_utils_js_1.getErrorMessage)(err)}`);
        }
    }
    /**
     * 按关键字搜索需求列表
    ...
    async searchRequirements(query) {
        try {
            ...
            throw new Error(`Failed to search requirements: ${(0, error_utils_js_1.getErrorMessage)(err)}`);
        }
    }
    /**
     * 获取当前配置的 MCP 服务器名称
     ...
     * @returns 解析后的需求列表
     */
    parseMarkdownRequirementList(text) {
        const results = [];
     ...
     */
    parseRequirementDetail(content) {
        const raw = this.extractContentJson(content);
     ...
     * @returns 解析后的需求详情对象
     */
    parseMarkdownRequirementDetail(text) {
        const lines = text.split('\n');
        let id = '';
        let title = '';
        let number;
            ...
            if (idx >= 0) {
                ...
                for (let i = idx + 1; i < lines.length; i++) {
                    if (isDocSection) {
                        ...
                        if (/^##\s+Attachments/i.test(lines[i]))
                            break;
                    }
                    else {
                        // 元数据章节：遇到同级 ## 标题时停止
