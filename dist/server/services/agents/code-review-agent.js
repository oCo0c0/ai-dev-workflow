...
class CodeReviewAgent {
    ...
    async act(context, action) {
        switch (action.type) {
            case 'analyze-quality':
                return await this.analyzeQuality(action.parameters);
            case 'check-security':
                return await this.checkSecurity(action.parameters);
            case 'verify-best-practices':
                return await this.verifyBestPractices(action.parameters);
            case 'complete':
    ...
    async verifyBestPractices(params) {
    ...
    assessReviewQuality(data) {
        ...
        if (data.length >= 5)
            score += 0.1;
        if (data.some((f) => f.suggestion))
            score += 0.1;
        return Math.min(score, 1.0);
    }
    async compileFinalResult(context) {
        const { state } = context;
        const qualityFindings = state.history[0]?.result.data || [];
        const securityFindings = state.history[1]?.result.data || [];
        const practiceFindings = state.history[2]?.result.data || [];
        ...
        const positives = [];
        if (criticalCount === 0)
            positives.push('No critical security issues found');
        if (bySeverity.high <= 2)
            positives.push('Few high-severity issues');
        if (bySeverity.medium <= 5)
            positives.push('Acceptable number of medium-severity issues');
...
function createCodeReviewAgent() {
