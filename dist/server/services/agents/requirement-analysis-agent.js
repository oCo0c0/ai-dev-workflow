...
exports.RequirementAnalysisAgent = void 0;
exports.createRequirementAnalysisAgent = createRequirementAnalysisAgent;
/**
 * 需求分析 Agent 实现
 */
class RequirementAnalysisAgent {
        ...
        if (iteration === 2) {
            return {
                ...
                nextAction: {
                    ...
                    parameters: {
                        requirement: task.input.requirement,
                        type: state.history[0]?.result.data?.type || 'feature'
                    }
                },
                confidence: 0.8
            };
    ...
    async act(context, action) {
        switch (action.type) {
            case 'analyze':
                return await this.analyzeRequirement(action.parameters);
            case 'detect-ambiguities':
                return await this.detectAmbiguities(action.parameters);
            case 'generate-criteria':
                return await this.generateAcceptanceCriteria(action.parameters);
            case 'complete':
     ...
     * 分析需求
     */
    async analyzeRequirement(params) {
     ...
     * 检测歧义
     */
    async detectAmbiguities(params) {
     ...
     * 生成验收标准
     */
    async generateAcceptanceCriteria(params) {
    ...
    async compileFinalResult(context) {
        ...
        // 从历史中提取结果
        const analysis = state.history[0]?.result.data;
        const ambiguities = state.history[1]?.result.data || [];
        const acceptanceCriteria = state.history[2]?.result.data || [];
        // 生成技术建议
        const technicalSuggestions = this.generateTechnicalSuggestions(task.input.requirement, analysis);
        // 识别依赖项
        const dependencies = this.identifyDependencies(task.input.requirement);
        return {
            requirementId: task.id,
            title: task.input.title || 'Untitled Requirement',
            description: task.input.requirement || '',
            analysis: analysis || {
                type: 'feature',
                complexity: 'medium',
     ...
     * 识别风险
     */
    identifyRisks(text, complexity, type) {
        const risks = [];
        if (complexity === 'high') {
     ...
     * 生成技术建议
     */
    generateTechnicalSuggestions(requirement, analysis) {
        const suggestions = [];
        // 基于需求内容生成特定建议
 ...
 * 创建需求分析 Agent 实例
 */
function createRequirementAnalysisAgent() {
    return new RequirementAnalysisAgent();
}
//# sourceMappingURL=requirement-analysis-agent.js.map