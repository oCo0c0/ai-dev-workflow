...
exports.getCoordinatorService = getCoordinatorService;
exports.executeAgentMode = executeAgentMode;
const coordinator_agent_js_1 = require("./coordinator-agent.js");
...
class CoordinatorService {
    ...
    initialized = false;
    constructor() {
        this.agentsService = new agents_service_js_1.AgentsService();
        this.coordinatorAgent = (0, coordinator_agent_js_1.createCoordinatorAgent)(this.agentsService);
    }
    static getInstance() {
    ...
    registerAgentCapabilities() {
        const registry = (0, agent_capability_js_1.getAgentRegistry)();
        // 需求分析Agent
        registry.register({
     ...
     */
    async executeAgentMode(requirement, workspace, context = {}) {
        // 确保已初始化
        this.initialize();
        // 提取配置参数
        ...
        const coordinator = (0, coordinator_agent_js_1.createCoordinatorAgent)(this.agentsService, config);
        // 使用协调Agent执行
    ...
    listAvailableAgents() {
        this.initialize();
        return (0, agent_capability_js_1.getAgentRegistry)().getAllAgents();
    }
    /**
     * 为特定任务推荐Agent
     */
    recommendAgents(taskDescription, minConfidence = 0.3) {
        this.initialize();
        return (0, agent_capability_js_1.getAgentRegistry)().getCandidates(taskDescription, {}, minConfidence);
    }
    /**
     * 向后兼容：保留原有的executeAgent接口
     * 如果需要单独执行某个Agent（非协调模式），仍可使用
     */
    async executeSingleAgent(agentType, taskId, inputData, options) {
 ...
 * 获取协调服务实例
 */
function getCoordinatorService() {
    return CoordinatorService.getInstance();
}
/**
 * 便捷函数：执行Agent模式
 */
async function executeAgentMode(requirement, workspace, context) {
