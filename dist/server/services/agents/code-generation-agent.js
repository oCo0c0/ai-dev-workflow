...
exports.createCodeGenerationAgent = createCodeGenerationAgent;
...
class CodeGenerationAgent {
    ...
    loopService = (0, loop_1.createLoopService)();
    defaultStyleConfig = {
        language: 'typescript',
        indentSize: 2,
        indentType: 'spaces',
    ...
    async act(context, action) {
        switch (action.type) {
            case 'generate-code':
                return await this.generateCode(action.parameters);
            case 'optimize-quality':
                return await this.optimizeQuality(action.parameters);
            case 'generate-tests':
    ...
    async observe(context, result) {
        ...
        const quality = await this.assessCodeQuality(result.data);
    ...
    async reflect(context, observation) {
        ...
        const code = context.state.history[context.state.history.length - 1]?.result.data;
     ...
     * 生成 TypeScript 代码
     */
    generateTypeScriptCode(requirement) {
        return `/**
 * ${requirement}
 *
...
export class RequirementHandler {
     ...
     * 生成 Python 代码
     */
    generatePythonCode(requirement) {
        return `"""
${requirement}
...
`;
     ...
     * 生成通用代码
     */
    generateGenericCode(requirement, language) {
        return `// ${requirement}
...
`;
    ...
    async optimizeQuality(params) {
        ...
        const result = await this.loopService.executeQualityLoop(config, async (improvements) => {
     ...
     * 应用改进
     */
    applyImprovement(code, improvement) {
...
`;
    ...
    improveErrorHandling(code) {
        ...
        if (!code.includes('try') && !code.includes('catch')) {
            return code.replace(/(return\s+[^;]+;)/g, 'try { $1 } catch (error) { console.error(error); throw error; }');
        }
        return code;
    }
    /**
...
`;
    ...
    async compileFinalResult(context) {
        ...
        const tests = state.history.find(h => Array.isArray(h.result.data))?.result.data || [];
        // 评估代码质量
        ...
        return {
            code,
            filePath: task.input.filePath || 'generated/RequirementHandler.ts',
            language: task.input.language || 'typescript',
            quality: {
                completeness: 0.85,
    ...
    async assessCodeQuality(code) {
        ...
        if (code.includes('/**') || code.includes('"""'))
            score += 0.1;
        if (code.includes('@param') || code.includes('Args:'))
            score += 0.05;
        // 错误处理检查
 ...
 * 创建代码生成 Agent 实例
 */
function createCodeGenerationAgent() {
    return new CodeGenerationAgent();
}
//# sourceMappingURL=code-generation-agent.js.map