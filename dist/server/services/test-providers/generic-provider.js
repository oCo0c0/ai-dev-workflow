...
exports.GenericTestProvider = void 0;
/**
 * 通用测试 Provider
 * @description 兜底实现，支持任意自定义测试命令。
 *   不做项目检测（总是返回 null），仅在用户手动指定时使用。
 */
class GenericTestProvider {
    type = 'generic';
    priority = 999;
    /**
     * 通用 Provider 不做自动检测
     */
    detect(_workspacePath) {
     ...
     * 生成可显示的项目信息
     */
    createProjectInfo(workspacePath) {
        return {
            type: 'generic',
     ...
     */
    listTestTargets(_workspacePath, _changedFiles) {
        return [];
    }
    /**
     * 获取执行命令
     */
    getRunCommand(framework, _targets, filter) {
...
exports.GenericTestProvider = GenericTestProvider;
