...
exports.PythonTestProvider = void 0;
...
class PythonTestProvider {
    ...
    detect(workspacePath) {
        ...
        for (const marker of pythonMarkers) {
            if (fs_1.default.existsSync(path_1.default.join(workspacePath, marker))) {
    ...
    listTestTargets(workspacePath, changedFiles) {
        ...
        for (const file of changedFiles) {
            ...
            if (normalized.endsWith('__init__.py') || normalized.endsWith('conftest.py'))
                continue;
            const moduleName = path_1.default.basename(normalized, '.py');
     ...
     * 检测 Python 构建工具/包管理器
     */
    detectBuildTool(workspacePath) {
        if (fs_1.default.existsSync(path_1.default.join(workspacePath, 'poetry.lock')))
            return 'poetry';
        if (fs_1.default.existsSync(path_1.default.join(workspacePath, 'pdm.lock')))
            return 'pdm';
        if (fs_1.default.existsSync(path_1.default.join(workspacePath, 'Pipfile')))
            return 'pipenv';
        if (fs_1.default.existsSync(path_1.default.join(workspacePath, 'uv.lock')))
            return 'uv';
        return 'pip';
    }
    /**
    ...
    detectFrameworks(workspacePath) {
        ...
        if (!pytestDetected && fs_1.default.existsSync(path_1.default.join(workspacePath, 'conftest.py'))) {
            pytestDetected = true;
            pytestConfigFile = 'conftest.py';
        }
        // requirements.txt 中查找 pytest 依赖
        if (!pytestDetected) {
            const reqPath = path_1.default.join(workspacePath, 'requirements.txt');
            if (fs_1.default.existsSync(reqPath)) {
                try {
                    const content = fs_1.default.readFileSync(reqPath, 'utf-8');
        ...
        if (!pytestDetected) {
            if (fs_1.default.existsSync(path_1.default.join(workspacePath, 'tests')) || fs_1.default.existsSync(path_1.default.join(workspacePath, 'test'))) {
                // 检查目录中是否有 test_*.py 文件
                const testDir = fs_1.default.existsSync(path_1.default.join(workspacePath, 'tests'))
                    ? path_1.default.join(workspacePath, 'tests')
                    : path_1.default.join(workspacePath, 'test');
                try {
                    const entries = fs_1.default.readdirSync(testDir);
     ...
     * 映射源文件到候选测试文件
     */
    mapSourceToTestFiles(sourcePath, moduleName) {
     ...
     * 生成 Pytest 命令
     */
    getPytestCommand(targets, filter) {
        let command = 'pytest';
     ...
     * 生成 unittest 命令
     */
    getUnittestCommand(targets, filter) {
        if (targets && targets.length > 0) {
     ...
     * 解析 Pytest 控制台输出
     */
    parsePytestOutput(output, exitCode) {
        const results = {
            framework: 'pytest',
     ...
     * 解析 unittest 控制台输出
     */
    parseUnittestOutput(output, exitCode) {
        const results = {
            framework: 'unittest',
            totalTests: 0, passed: 0, failed: 0, skipped: 0,
            duration: 0, suites: [],
...
exports.PythonTestProvider = PythonTestProvider;
//# sourceMappingURL=python-provider.js.map