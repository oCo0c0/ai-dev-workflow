...
const GLOBAL_SETTINGS_FILE = path_1.default.join(os_1.default.homedir(), '.claude.json');
/**
 * MCP 配置服务类
 *
...
class MCPConfigService {
    ...
    constructor(settingsFile) {
        ...
        this.claudeDir = path_1.default.dirname(this.settingsFile);
    }
    /**
     * 确保 Claude 配置目录存在
     ...
     */
    ensureClaudeDir() {
        if (!fs_1.default.existsSync(this.claudeDir)) {
            fs_1.default.mkdirSync(this.claudeDir, { recursive: true });
        }
    }
    /**
     * 从磁盘加载 Claude 设置文件
    ...
    saveSettings(settings) {
        ...
        fs_1.default.writeFileSync(this.settingsFile, JSON.stringify(settings, null, 2), 'utf-8');
    }
    /**
     * 列出所有已配置的 MCP 服务器
     *
     ...
     */
    loadMcpServersFromFile(filePath) {
    ...
    testConnection(name, timeoutMs = 5000) {
        ...
        return new Promise((resolve) => {
            try {
                ...
                resolve({ status: 'error', message: `Failed to spawn process: ${(0, error_utils_js_1.getErrorMessage)(err)}` });
            }
        });
    }
    /**
