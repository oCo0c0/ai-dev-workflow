...
/** 预热的共享实例（id → 实例） */
const instances = new Map<string, RequirementSourceAdapter>();
 ...
 */
export function registerRequirementSource(factory: AdapterFactory): void {
 ...
 */
export function bindServer(serverName: string, adapterId: string): void {
    bindings.set(serverName, adapterId);
}

/** 解除显式绑定 */
export function unbindServer(serverName: string): void {
 ...
 */
export function getAdapter(id: string): RequirementSourceAdapter | undefined {
 ...
 */
export function listAdapters(): RequirementSourceAdapter[] {
 ...
 */
export function listCatalogAdapters(): RequirementSourceAdapter[] {
 ...
 */
export function resolveAdapter(serverName: string, config: MCPServerConfig | undefined): RequirementSourceAdapter {
    // 1. 显式绑定
    ...
    if (config) {
        for (const adapter of instances.values()) {
            try {
                if (adapter.matchServer(config)) return adapter;
