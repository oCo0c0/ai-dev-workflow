...
export class GithubAdapter {
     ...
     */
    buildDetailArgs(normalizedInput, config) {
        ...
        if (num) {
            ...
            throw new Error(`GitHub issue 需要仓库限定：请输入 owner/repo#${num[1]}，或在 MCP server 的 env 中配置 GITHUB_REPOSITORY=owner/repo`);
        }
...
function inferPriority(labels) {
    ...
    if (/\bp0\b|critical|urgent|blocker/.test(joined))
        return 'high';
    if (/\bp2\b|low|minor/.test(joined))
        return 'low';
    return 'medium';
}
