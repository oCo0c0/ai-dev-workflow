...
export class MinerUClient {
    ...
    async parseFile(filePath, options) {
        if (!this.isConfigured())
            return { success: false, error: 'MinerU service is not configured' };
    ...
    async submitTask(filePath, options) {
        if (!this.isConfigured())
            return { task_id: '', error: 'MinerU service is not configured' };
        if (!fs.existsSync(filePath))
    ...
    async getTaskStatus(taskId) {
        if (!this.isConfigured())
            return { task_id: taskId, status: 'failed', error: 'MinerU service is not configured' };
        try {
    ...
    parseResponse(body) {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                result.markdown = data.map((item) => MinerUClient.extractMarkdown(item)).filter(Boolean).join('\n\n---\n\n');
                return result;
            }
...
//# sourceMappingURL=mineru-client.js.map